...
function badgeos_custom_metaboxes(array $meta_boxes)
{
    ...
    $query = new WP_Query(array('post_type' => 'badges', 'posts_per_page' => -1));
    $badge_list = array();
    $posts = $query->get_posts();
    foreach ($posts as $post) {
        $badge_list[] = array('name' => ($post->post_title), 'value' => ($post->ID));
    }

    $user_list = array();
    $users = get_users(array());
    foreach ($users as $user) {
        $user_list[] = array('name' => ($user->display_name), 'value' => ($user->ID));
    }
    ...
    $meta_boxes[] = array(
        ...
        'fields' => array(
            ...
            array(
                ...
                'type' => 'select',
                'options' => ($badge_list),
                'default' => get_post_meta($post_id, '_badgeos_nomination_achievement_id', true),
            ),
            /*
            array(
                'name' => __( 'Achievement ID to Award', 'badgeos' ),
    ...
    $meta_boxes[] = array(
        ...
        'fields' => array(
            ...
            array(
                'name' => __('Preuve', 'badgeos'),
                //'desc' => '<a href="' . esc_url(admin_url('post.php?post=' . get_post_meta($post_id, '_badgeos_nomination_achievement_id', true) . '&action=edit')) . '">' . __('View Achievement', 'badgeos') . '</a>',
                'id' => $prefix . 'nomination_preuve',
                'type' => 'file',
                //'options' => ($badge_list),
                //'default' => get_post_meta($post_id, '_badgeos_nomination_achievement_id', true),
            ),
